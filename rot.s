    ...
    mov     %rcx,       %r12           # this is my j, reset to i
        .innerLoop:
           ...
           mov     $7,         %r9 
            ...
            movw    (%rdx,%rcx,2),   %r8w         # r8 contains arr[N-1-j][i]
            movw    %r8w,         (%r11, %r12, 2) #   a[i][j] = a[N - 1 - j][i];   (((((((1)))))))

            movw     (%rbx,%r14,2),  %r8w         # r8 contains arr[N-1-i][N-1-j]
            movw     %r8w,        (%rdx,%rcx,2)   #   a[N - 1 - j][i] = a[N - 1 - i][N - 1 - j];
            
            movw    (%r15,%r9,2),   %r8w        # r8 contains arr[j][N-1-i]
            movw    %r8w,       (%rbx,%r14,2)   # a[N - 1 - i][N - 1 - j] = a[j][N - 1 - i];

            movw    %r13w,    (%r15,%r9,2)        #   a[j][N - 1 - i] = temp;
